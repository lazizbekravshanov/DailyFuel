// Share card images, drawn once at build time: /og/us-2026-09-14.png and one
...
import { cardDate, cardFor, cardKeys, renderCard } from "../../lib/og.ts";

export const getStaticPaths = (() => {
  const site = getSite();
  const date = cardDate(site);
  return cardKeys(site).map((key) => ({ params: { card: `${key}-${date}` }, props: { key } }));
